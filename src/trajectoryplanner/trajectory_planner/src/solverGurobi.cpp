...
{
  std::cout << "========== In myCallback of solvergurobi" << std::endl;
...
{  // This function is called periodically along the optimization process.
  ...
  // std::cout << "========== In callback of solvergurobi" << std::endl;
  if (should_terminate_ == true)
  {
    std::cout << "========== In myCallback of solvergurobi Should Terminate = True" << std::endl;
...
bool SolverGurobi::genNewTraj()
{
  ...
  trials_ = 0;
    ...
    // std::cout << "Number of rows= " << polytopes_[0].b().rows() << std::endl;
...
bool SolverGurobi::callOptimizer()
{
  ...
  //     m.optimize();
  //     optimstatus = m.get(GRB_IntAttr_Status);
  //   }
  ...
  //     m.write("model.ilp");
  //   }
  // std::cout << optimstatus << " " << GRB_OPTIMAL << " " << GRB_INF_OR_UNBD << " " << GRB_NUMERIC << " " << GRB_INTERRUPTED << std::endl;
