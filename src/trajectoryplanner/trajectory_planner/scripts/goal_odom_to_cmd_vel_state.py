...
class GoalToCmdVel:
    ...
    def printTime(self, time):
        print("Mins: ",(time/60)%60, end="")
        print(" Secs: ",time%60, end="")
        print(" Total Secs: ", time)
    ...
    def odomCB(self, msg):
        ...
        if hasattr(self, 'planMode') and self.planMode != 3:
            if(self.first_run):
                self.velArr = []
                self.odomArr = []
            ...
            self.velArr.append(msg.twist.twist.linear.x)
            self.odomArr.append(msg)
            ...
            print("Total distance traveled is "+ str(self.total_distance) +"m")
            self.previous_x = msg.pose.pose.position.x
            self.previous_y = msg.pose.pose.position.y
        ...
        if hasattr(self, 'plotFlag') and self.plotFlag:
            self.endTime = rospy.get_rostime()
            self.timeDiff = self.endTime - self.startTime
            self.printTime(self.timeDiff.secs)
            path = os.path.abspath('testPlot.bag')
            print(path)
            bag = rosbag.Bag(path, mode='w')
            try:
                for i in self.odomArr:
                    # f = Float32()
                    # f.data = i
                    bag.write('odom', i)
            ...
            # with open('testPlot.csv','w') as f:
            #     write = csv.writer(f) 
            #     write.writerow(self.velArr)
            # self.ax1.plot(self.velArr)
    ...
    def planModeCB(self, i):
        # print("i = ",i)
        # print(i.data)
        if hasattr(self, 'planMode'):
            self.prevPlanMode = self.planMode
            if i.data == 3 and self.prevPlanMode != 3:
                self.plotFlag = True
        self.planMode = i.data
...
def startNode():
    ...
    # rospy.Subscriber("faster/plan_state", Int8, c.planModeCB, queue_size=1)
